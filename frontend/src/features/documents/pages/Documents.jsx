...
import { getDocsByUser, addDoc } from "../../../lib/docsStore";
import Card from "../../../components/Card";
import Badge from "../../../components/Badge";
import EmptyState from "../../../components/EmptyState";
import { DOCUMENT_TYPES } from "../../../mock/mockData";
...
export default function Documents() {
  ...
  const refresh = () => setHistory(currentUser ? getDocsByUser(currentUser.id) : []);
  useEffect(refresh, [currentUser]);

  const generate = () => {
    addDoc({
      userId: currentUser.id,
      name: { fr: current.name.fr, en: current.name.en },
      reason, period,
      date: new Date().toLocaleDateString(lang === "fr" ? "fr-FR" : "en-GB"),
      status: "pending",
    });
    setReason(""); setPeriod(""); refresh();
  };

  const field = { width: "100%", height: 44, marginTop: 6, marginBottom: 14, borderRadius: 10, border: "1px solid var(--line)", background: "var(--field)", color: "var(--ink)", padding: "0 12px", outline: "none", fontFamily: "inherit", fontSize: 14 };
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginTop: 20 }}>
        {DOCUMENT_TYPES.map((d) => {
          ...
          return (
            <button key={d.id} onClick={() => setSelected(d.id)} style={{ textAlign: "left", cursor: "pointer", background: "var(--surface)", padding: 18, borderRadius: 14, border: `1px solid ${active ? "var(--gold)" : "var(--line)"}`, fontFamily: "inherit" }}>
              <div style={{ width: 42, height: 42, borderRadius: 11, display: "flex", alignItems: "center", justifyContent: "center", background: active ? "var(--gold)" : "var(--gold-tint)", color: active ? "var(--on-gold)" : "var(--gold-deep)" }}><Icon size={20} /></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 16 }}>
        <Card>
          ...
          <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="…" style={field} />
          <label style={{ fontSize: 13, fontWeight: 500, color: "var(--ink)" }}>{t("docs.field.period")}</label>
          <input value={period} onChange={(e) => setPeriod(e.target.value)} placeholder="…" style={{ ...field, marginBottom: 0 }} />
          <button onClick={generate} style={{ marginTop: 18, width: "100%", height: 46, borderRadius: 9, border: "none", background: "var(--gold)", color: "var(--on-gold)", fontWeight: 600, fontSize: 14.5, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: "inherit" }}>
      ...
      <Card style={{ marginTop: 10, padding: 0 }}>
        {history.length === 0 ? <EmptyState title={t("docs.empty")} /> : history.map((d, i) => (
          <div key={d.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", borderTop: i ? "1px solid var(--line)" : "none" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "var(--muted)" }}>{d.date}{d.reason ? ` · ${d.reason}` : ""}</div>
            </div>
