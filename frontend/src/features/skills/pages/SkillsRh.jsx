...
import { Star, BadgeCheck, Layers, Users, Plus, Trash2, Sparkles } from "lucide-react";
...
import {
  ...
  getCompetenceRadar, validateEvaluation, getMissingPostes, syncPostes,
  createMetier, deleteMetier, createCompetence, addCompetenceRequise, deleteCompetenceRequise,
} from "../../../lib/api";

const NIVEAUX = ["Junior", "Opérationnel", "Confirmé", "Senior", "Expert"];
...
export default function SkillsRh() {
  ...
  const { data, loading, error, reload } = useAsync(async () => {
    const [pend, metiers, emps, cat, missing] = await Promise.all([
      getPendingEvaluations(), getMetiers(), getEmployees({ page_size: 100 }),
      getCompetences(), getMissingPostes().catch(() => ({ data: [] })),
    ]);
    return { pending: (pend && pend.data) || [], metiers: (metiers && metiers.data) || [],
             employees: (emps && emps.data) || [], catalogue: (cat && cat.data) || [],
  ...
  const validate = async (id) => {
    if (!exp[id]) return;
    setBusy(id);
    try { await validateEvaluation(id, exp[id]); reload(); } catch (e) { fail(e); } finally { setBusy(null); }
  };
  ...
  const openRadar = async (mat) => {
    if (!mat) { setRadar(null); return; }
    try { const r = await getCompetenceRadar(mat); setRadar({ emp: mat, data: (r && r.data) || {} }); }
    catch (e) { setRadar({ emp: mat, data: {} }); }
  };
  ...
  const radarData = (radar?.data?.items || []).map((i) => ({ competence: i.competence, attendu: i.attendu, actuel: i.actuel }));
  ...
  return (
    <div>
      ...
      <AsyncBoundary loading={loading} error={error} onRetry={reload}>
        {/* File de validation (inclut les compétences nouvellement proposées) */}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <Card>
            ...
            <select onChange={(e) => openRadar(e.target.value)} style={{ ...field, width: "100%", marginBottom: 12 }} defaultValue="">
              <option value="">{t("skills.pickCollab")}</option>
              {(data?.employees || []).map((emp) => (
                <option key={emp.id} value={emp.id}>{`${emp.prenom || ""} ${emp.nom || ""}`.trim() || emp.id} — {emp.poste || "—"}</option>
              ))}
            </select>
            {radar && (radarData.length === 0 ? (
              ...
              <>
                ...
                {(radar.data.gaps || []).length > 0 && (
                  <div style={{ marginTop: 8, fontSize: 12.5, color: "var(--muted)" }}>
                    {t("skills.gaps")} : {radar.data.gaps.map((g) => `${g.competence} (${g.actuel}/${g.attendu})`).join(", ")}
                  </div>
                )}
              </>
            ))}
