...
const TAB_KEYS = ["turnover", "burnout", "desengagement", "simulation"];
...
export default function PredictiveAnalytics() {
  ...
  for (const e of (data?.employees || [])) {
    deptById[e.id] = e.department || "Non assigné";
  }
  ...
  const buildDeptDist = (riskData) => {
    ...
    for (const r of riskData) {
      ...
      const d = deptById[r.employee_id] || "Non assigné";
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        <h1 className="font-display" style={{ fontSize: 28, fontWeight: 600, color: "var(--ink)", margin: 0 }}>Analytics IA</h1>
        <div style={{ display: "flex", gap: 16, borderBottom: "1px solid var(--line)" }}>
          <div style={tabStyle(activeTab === "turnover")} onClick={() => setActiveTab("turnover")}>Départ (Turnover)</div>
          <div style={tabStyle(activeTab === "burnout")} onClick={() => setActiveTab("burnout")}>Burnout</div>
          <div style={tabStyle(activeTab === "desengagement")} onClick={() => setActiveTab("desengagement")}>Désengagement</div>
          <div style={tabStyle(activeTab === "simulation")} onClick={() => setActiveTab("simulation")}>Simulation RH</div>
        </div>
      </div>
      ...
      <AsyncBoundary loading={loading} error={error} onRetry={reload}>
        ...
        {activeTab === "turnover" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <Card>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink)", marginBottom: 12 }}>Distribution du Risque de Départ</div>
              <div style={{ height: 260 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={distTurnover} margin={{ top: 6, right: 10, left: -18, bottom: 0 }}>
                    ...
                    <Tooltip />
                    <Bar dataKey="n" radius={[4, 4, 0, 0]} name="Employés">
            ...
            <Card>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink)", marginBottom: 12 }}>Risques par Département (Top 5)</div>
              <div style={{ height: 260 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={deptTurnover} layout="vertical" margin={{ top: 6, right: 10, left: 10, bottom: 0 }}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="high" stackId="a" fill="var(--danger)" name="Élevé" radius={[0, 0, 0, 0]} />
                    <Bar dataKey="mid" stackId="a" fill="#9a6b12" name="Modéré" radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </Card>
            ...
            <Card style={{ gridColumn: "1 / -1" }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink)", marginBottom: 12 }}>Comparaison: Risque de Départ vs Désengagement</div>
              <div style={{ height: 260 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={compareToDesengagement} margin={{ top: 6, right: 10, left: -18, bottom: 0 }}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="turnover" fill="var(--gold)" radius={[4, 4, 0, 0]} name="Risque de Départ" />
                    <Bar dataKey="desengagement" fill="#5865F2" radius={[4, 4, 0, 0]} name="Désengagement" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </Card>
        ...
        {activeTab === "burnout" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {/* Graph 1: Distribution */}
            <Card>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink)", marginBottom: 12 }}>Distribution du Risque de Burnout</div>
              <div style={{ height: 260 }}>
                <ResponsiveContainer width="100%" height="100%">
            ...
            <Card style={{ gridColumn: "1 / -1" }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink)", marginBottom: 12 }}>Historique global de l'Absentéisme (%)</div>
              <div style={{ height: 260 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={seriesAbs} margin={{ top: 6, right: 10, left: -18, bottom: 0 }}>
                    ...
                    <YAxis stroke="var(--muted)" fontSize={12} tickLine={false} axisLine={false} />
                    <Tooltip />
                    <Line type="monotone" dataKey="v" stroke="var(--gold)" strokeWidth={3} name="Taux d'absentéisme" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </Card>
        ...
        {activeTab === "desengagement" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {/* Graph 1: Distribution */}
            <Card>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink)", marginBottom: 12 }}>Distribution du Risque de Désengagement</div>
              <div style={{ height: 260 }}>
                <ResponsiveContainer width="100%" height="100%">
